...
test('document seeks reproduce pixels; complete cut stays addressable; no audio stacking',async({page})=>{
  ...
  const result=await page.evaluate(()=>{
    ...
    const frames=[74,79,87,90,94,101,111,119,124,197,239].map(t=>{app.seek(t);const first=art.toDataURL();app.seek(260);app.seek(t);return first===art.toDataURL();});
    ...
    const active=app.sound.diagnostics.activeSources;app.sound.stop();return {frames,active,stopped:app.sound.diagnostics.activeSources};
  });
  expect(result.frames.every(Boolean)).toBe(true);expect(result.active).toBe(1);expect(result.stopped).toBe(0);expect(errors).toEqual([]);
});
...
test('single file loads offline; mobile preserves image and caption bounds',async({page})=>{
  ...
  await expect(page.locator('#caption')).not.toBeEmpty();const frame=await page.locator('#art').boundingBox(),caption=await page.locator('#caption').boundingBox();
