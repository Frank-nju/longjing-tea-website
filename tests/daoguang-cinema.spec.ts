...
test('document seeks reproduce pixels; complete cut stays addressable; no audio stacking',async({page})=>{
  ...
  const result=await page.evaluate(()=>{
    ...
    const active=app.sound.diagnostics.activeSources;app.sound.stop();return {frames,humenChanged:humenEarly!==humenLate,humenStable:humenEarly===humenReplay,active,stopped:app.sound.diagnostics.activeSources};
  });
  expect(result.frames.every(Boolean)).toBe(true);expect(result.humenChanged).toBe(true);expect(result.humenStable).toBe(true);expect(result.active).toBe(1);expect(result.stopped).toBe(0);expect(errors).toEqual([]);
});
